...
The dataset is stored as a `tme_data` object, which has the following attributes that can be broken down into two categories:
...
: (\#tab:table2) Data attributes in the `tme_data` object.
...
: (\#tab:table3) Metadata attributes in the `tme_data` object.

Data attributes are matrices or dataframes containing data from the study, and metadata attributes describe the data in the study. 
Both can be accessed in the same manner; to view the gene expression data for this dataset use this:
```{r, eval=FALSE} #> not evaluated because of knitr kable call
View(res[[1]]$expression)
```
```{r, echo=FALSE}
knitr::kable(head(res[[1]]$expression[,1:2]))
```
